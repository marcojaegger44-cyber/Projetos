...
def testar_exclusao():
    print("=== TESTE DE EXCLUSÃO DE CLIENTES ===")
    ...
    print(f"\n2. Testando exclusão do cliente: {cliente_nome} (ID: {cliente_id})")
    
    try:
        remover_cliente(cliente_id)
        print(f"   ✅ Cliente '{cliente_nome}' excluído com sucesso!")
    ...
    if cliente_ainda_existe:
        print(f"   ❌ Cliente '{cliente_nome}' ainda existe no banco!")
    else:
        print(f"   ✅ Cliente '{cliente_nome}' foi removido com sucesso!")
